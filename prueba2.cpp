...
struct Pila{
    ...
    Pila(){ id = 0; proceso = ""; }
    Pila(int nID, string nProceso){ id = nID; proceso = nProceso; }
};
...
void modifyList(Nodo*& lista, int modifyId, int newPriority){
    if(newPriority < 1 || newPriority > 10){ cout<<"Prioridad no válida\n"; return; }
    Nodo* temp = lista;
...
void addToPila(int id, string proceso){
    if(top < MAX - 1){
        top++;
        memoria[top] = new Pila(id, proceso);
    }else{
        cout<<"La memoria está llena\n";
    }
}

void removeFromPila(){
    if(top == -1){ cout<<"No hay procesos en la memoria\n"; }
    else{
        cout<<"Proceso retirado: "<<memoria[top]->proceso<<"\n";
        delete memoria[top];
        top--;
    }
}
...
void showPila(){
    if(top == -1) cout<<"La pila está vacía.\n";
    else{
        for(int i = top; i >= 0; i--){
            cout<<"ID: "<<memoria[i]->id<<"\nProceso: "<<memoria[i]->proceso<<"\n";
        }
    }
}
        ...
        switch(opcion){
            case 1:
                if(!estaVacia()){
                    ...
                    addToPila(proc.id, proc.proceso);
                    desencolarCPU();
                }else{
    ...
    do{
        ...
        switch(opcion){
            case 1:
                ...
                removeFromPila();
                addToHist(historial, "Proceso ID: "+to_string(id));
                break;
            case 2: {
                string name;
...
int main(){
    ...
    do{
        ...
        switch(opcion){
            ...
            case 6: showPila(); break;
            case 7: showHist(historial); break;
